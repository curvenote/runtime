...
const dangerousEvaluatationMiddleware: Middleware = (store) => (next) => (action) => {
  ...
  switch (action.type) {
    ...
    case COMPONENT_EVENT: {
      const { id, component, name, values } = (action as ComponentEventAction).payload;
      const evaluated = store.dispatch(
        dangerouslyEvaluateState({
          id: component,
          name,
          values,
        }),
