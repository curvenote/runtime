import { DefineSpec, Spec, SpecProperty, SpecEvent } from './types';
...
export function getSpecFromDefinition(specDefinition: DefineSpec): Spec {
  const spec: Spec = {
    ...
    ...specDefinition,
    properties: forEachObject(specDefinition.properties, ([name, prop]) => [
      name,
      {
        name,
        type: prop.type,
    ...
    events: forEachObject(specDefinition.events, ([name, evt]) => [
      name,
      {
        name,
        args: evt.args ?? [],
  ...
  Object.entries(spec.properties).forEach(([name, prop]) => {
    ...
    prop.args.forEach((arg) => {
      ...
      throw new Error(
        `Property spec: "${name}" has unrecognized argument: "${arg}". It must be a property of the spec.`,
      );
    });
  });
  return spec;
